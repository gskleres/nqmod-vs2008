...
function PopulateTakeChoices()	
	...
	local pActivePlayer = Players[iActivePlayer];
	local iBullyGoldInflueneGained = pActivePlayer:GetMinorFriendshipGainBullyGoldSuccess();
	if (iBullyGoldInflueneGained > 0) then
		buttonText = Locale.Lookup("TXT_KEY_POPUP_MINOR_BULLY_GOLD_AMOUNT_GAIN_INFLUENCE", iBullyGold, iBullyGoldInflueneGained);
	else
	...
	-- end NQ_MINOR_FRIENDSHIP_GAIN_BULLY_GOLD_SUCCESS_FROM_POLICIES
