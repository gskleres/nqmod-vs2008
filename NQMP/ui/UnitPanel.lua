...
function TipHandler( control )
	...
	if (bBuild) then
		...
		if (pFeature) then
			...
			-- begin NQ_FOOD_FROM_CHOPS - this is interwoven with the previous code, not separated
			...
			if (iFeatureProduction > 0 and iFeatureFood > 0) then
				strClearFeatureString = strClearFeatureString .. Locale.ConvertTextKey("TXT_KEY_BUILD_FEATURE_PRODUCTION_AND_FOOD", iFeatureProduction, iFeatureFood);
			elseif (iFeatureProduction > 0) then
